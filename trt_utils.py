...
def create_image_stream(
    ...
    channel = int(input_shapes[0])
    height = int(input_shapes[1])
    width = int(input_shapes[2])
...
def create_tensorrt_engine(onnx_path, engine_type, input_shapes, dynamic, calibrator=None):
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(
        ...
        config.max_workspace_size = 1 << 29  # 1GB
        config.avg_timing_iterations = 5

        inputs = [network.get_input(i) for i in range(network.num_inputs)]
        profile = None
        if dynamic:
            input_shapes = [int(dim) for dim in input_shapes]
            profile = builder.create_optimization_profile()
            for inp in inputs:
                profile.set_shape(inp.name, [1, *input_shapes], [2, *input_shapes], [4, *input_shapes])
            config.add_optimization_profile(profile)

        if engine_type == "fp16" and builder.platform_has_fast_fp16:
            config.set_flag(trt.BuilderFlag.FP16)
            # builder.fp16_mode = True
        elif engine_type == "int8" and builder.platform_has_fast_int8:
        ...
        print("[ONNX2TRT] INFO: Engine build finished!")
    return engine
...
def allocate_buffers(engine):
    ...
    for binding in engine:
        ...
        # print("[ONNX2TRT] INFO: engine.max_batch_size: ", engine.max_batch_size)
        # print("[ONNX2TRT] INFO: get_binding_shape: ", engine.get_binding_shape(binding))
...
def evaluate_engine(onnx_path, engine, image_stream):
    ...
    while len(image_data) > 0:
        ...
        # print("onnx_outputs if NAN: ", [np.isnan(onnx_outputs[i]).sum() for i in range(len(onnx_output_names))])
        # for onnx_output in onnx_outputs:
        #     onnx_output[np.isnan(onnx_output)] = 0         
        onnx_outputs = {
        ...
        # print("trt_outputs if NAN: ", [np.isnan(trt_outputs[i]).sum() for i in range(len(trt_output_names))])
        # for trt_output in trt_outputs:
        #     trt_output[np.isnan(trt_output)] = 0  
