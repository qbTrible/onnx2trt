...
def main():
    ...
    """ usage: python3 onnx2trt.py --onnx dynamic_yolov5s.onnx --trt_engine ./dynamic_v5Lite-640-int8.trt --engine_type "int8" --calib_dir ./val2017 --calib_algo TRTEntropy --dynamic  """
    ...
    # onnx_model, check = simplify(onnx_model, dynamic_input_shape=True,
    #                 input_shapes={"images":[1, 3, 640, 640]}) # simplify 
    ...
    if engine_type == "int8":
        ...
        image_stream = create_image_stream(
            calib_dir, INPUT_SHAPES[0][1:], means, stds, pixel_type, channel_order
        )
        ...
        for calibrator_type in search_types:
            ...
            engine = create_tensorrt_engine(onnx_path, engine_type, INPUT_SHAPES[0][1:], dynamic, calibrator)
        ...
        final_engine = create_tensorrt_engine(onnx_path, engine_type, INPUT_SHAPES[0][1:], dynamic)
        if calib_dir != None:
            image_stream = create_image_stream(
                calib_dir, INPUT_SHAPES[0][1:], means, stds, pixel_type, channel_order
            )
    ...
    dir_name = os.path.dirname(trt_engine) if os.path.dirname(trt_engine) != "" else "."
